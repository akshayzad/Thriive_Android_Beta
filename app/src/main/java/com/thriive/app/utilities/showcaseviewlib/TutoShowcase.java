...
package com.thriive.app.utilities.showcaseviewlib;
...
import com.thriive.app.utilities.showcaseviewlib.shapes.ReactAngle;
import com.thriive.app.utilities.showcaseviewlib.shapes.RoundRect;
...
public final class TutoShowcase {
    ...
    public static class ViewActions {
        ...
        public ShapeViewActionsEditor addRectAngle() {
            return addRectAngle(DEFAULT_ADDITIONAL_RADIUS_RATIO);
        }

        private ShapeViewActionsEditor addRectAngle(float defaultAdditionalRadiusRatio) {
            view.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
                @Override
                public boolean onPreDraw() {
                    addRectAngleOnView(defaultAdditionalRadiusRatio);
                    view.getViewTreeObserver().removeOnPreDrawListener(this);
                    return false;
                }
            });
        ...
        private void addRectAngleOnView(float additionalRadiusRatio) {
            Rect rect = new Rect();
            view.getGlobalVisibleRect(rect);
            ...
            ReactAngle roundRect = new ReactAngle(x, y, width, height);
            roundRect.setDisplayBorder(settings.withBorder);
            tutoShowcase.tutoView.addReactAngle(roundRect);
            addClickableView(rect, settings.onClickListener, additionalRadiusRatio);
            tutoShowcase.tutoView.postInvalidate();
        }

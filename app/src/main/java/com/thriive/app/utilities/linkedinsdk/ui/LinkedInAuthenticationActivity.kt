...
import com.thriive.app.utilities.linkedinsdk.di.DependencyGraph
...
import com.thriive.app.utilities.linkedinsdkutil.linkedinsdk.domain.utils.LinkedInConst.RESPONSE_TYPE_VALUE
import com.thriive.app.utilities.linkedinsdkutil.linkedinsdk.domain.utils.LinkedInConst.getAuthorizationUrl
import com.thriive.app.utilities.linkedinsdkutil.linkedinsdk.domain.utils.RequestHandler
import com.thriive.app.utilities.linkedinsdkutil.linkedinsdk.presentation.linkedin_authentication.models.LinkedInAuthenticationState
...
import com.thriive.app.utilities.linkedinsdkutil.linkedinsdk.presentation.linkedin_authentication.viewmodel.LinkedInAuthenticationViewModel
...
class LinkedInAuthenticationActivity : AppCompatActivity() {
    ...
    private val clientId by lazy { intent.getStringExtra(LinkedInBuilder.CLIENT_ID) }
    private val clientSecretKey by lazy { intent.getStringExtra(LinkedInBuilder.CLIENT_SECRET_KEY) }
    private val redirectUri by lazy { intent.getStringExtra(LinkedInBuilder.REDIRECT_URI) }
    private val state by lazy { intent.getStringExtra(LinkedInBuilder.STATE) }


    private val accessTokenOnlyRequest by lazy { intent.getBooleanExtra(LinkedInBuilder.ACCESS_TOKEN_ONLY, false) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val dependencyGraph = createDependencyGraph(clientId.toString(), clientSecretKey.toString(), redirectUri.toString(), accessTokenOnlyRequest)
        ...
        web_view_linkedin_login.webViewClient = object : WebViewClient() {
            ...
            override fun shouldOverrideUrlLoading(view: WebView, authorizationUrl: String): Boolean {
                setProgressDialogVisibility(true)
                if (authorizationUrl.contains("login-cancel?")) {
                    finish()
                } else if (authorizationUrl.startsWith(redirectUri.toString())) {
        ...
        val authUrl = getAuthorizationUrl(clientId.toString(), redirectUri.toString(), state.toString())
        web_view_linkedin_login.loadUrl(authUrl)
//        if (authUrl.contentEquals("user_cancelled_login"))
//        {
//
//            finish()
//        }

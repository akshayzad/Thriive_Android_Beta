...
public class SelectedProposeSlotAdapter extends RecyclerView.Adapter<SelectedProposeSlotAdapter.RecyclerAdapterHolder> {
    ...
    public SelectedProposeSlotAdapter(Context context, Fragment fragment, ArrayList<CommonRequestTimeSlots.EntitySlotsListPOJO> childArrayList){
        this.context = context;
        this.fragment = fragment;
        this.childArrayList = childArrayList;
    }
    @Override
    public SelectedProposeSlotAdapter.RecyclerAdapterHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.layout_selected_time_slots, parent, false);

        return new SelectedProposeSlotAdapter.RecyclerAdapterHolder(itemView);
    }

    @SuppressLint("NewApi")
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    @Override
    public void onBindViewHolder(final SelectedProposeSlotAdapter.RecyclerAdapterHolder holder, int position) {
...
//                ((NotificationListActivity)  activity).getSelectedTimeSlot(item ,position);
                ((NotificationFragment)  fragment).getSelectedTimeSlot(item ,position);
        ...
        holder.layout_remove_slot.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                 Activity activity = Utility.unwrap(view.getContext());
//                ((NotificationListActivity)  activity).removeSelectedTimeSlot(position);
                ((NotificationFragment)  fragment).removeSelectedTimeSlot(position);
                notifyDataSetChanged();
            }
        });
    }
